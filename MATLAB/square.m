...
% ---------------------------------------------------------------
%
% This function creates a single-cycle square waveform for a given note frequency
% using precomputed wavetable banks. Only odd harmonics are included to form
% the square wave. The waveform is phase-aware and ready for continuous playback.
%
% Inputs:
...
%   - Only odd harmonics are used to synthesize the square wave.
% ---------------------------------------------------------------
...
for b = 1:length(banks)
    ...
    % Harmonic vector and amplitudes (only odd harmonics, scale 1/k)
    k = 1:number_of_harmonics;
    a = mod(k,2) ./ k;  % odd harmonics only
